...
**無補正だと音声が映像より 60〜140 ms 先行します。** 映像側は Desktop Duplication の取得と
filter graph の起動ぶんだけ遅れるのに対し、DirectShow の音声にはその遅れが無く、
`aresample=first_pts=0` が音声を強制的に 0 秒へ寄せるためです。ドリフトではなく一定量のずれなので、
`-itsoffset` で音声を遅らせれば解消します。

白フラッシュと 1kHz ビープを**同一タイムスタンプで持つ基準クリップ**を再生しながら録画し、
録画結果の音声ピーク位置と最大輝度フレームの差を測った結果:

| `-itsoffset` | 実測のずれ（負＝音声が先行） |
| --- | --- |
| 0 ms | −60 / −113 / −137 ms |
| **80 ms** | **−40 / −23 / −20 / −20 / −57 ms**（最も安定、σ≈15 ms） |

これを踏まえて既定を **100 ms** にしています。値は環境（音声デバイス・解像度・エンコーダ）で
変わるので、気になる場合は「音声オフセット (ms)」で調整してください。正の値で音声が遅れます。
...
`synctest.mp4` は **毎秒ちょうど、画面全体の白フラッシュと 1kHz のビープが同時に出る**
クリップです。両者は同じ式から生成しているのでサンプル単位で一致しています。
画面下部を走るシアンのバーはコマ落ちの目視用です。

`analyze_capture.py` は録画結果の輝度波形と音声振幅波形を ±1 秒ずらしながら相関を取り、
いちばん一致する位置をずれ量として出します。1 回のイベントではなく 29 回ぶんを使うので、
単発のカチンコより安定します。
...
> 既知のずれ（−100 / +150 / −250 ms）を与えたファイルで、較正後は誤差 0 ms で復元することを確認済みです。
