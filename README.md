...
音声が映像より早い／遅いときは「音声オフセット (ms)」で `-itsoffset` を付けます
（正の値で音声を遅らせる）。

**ずれ量は環境によって変わるので、既定は 0 です。** 実測して決めてください（次の節）。
以前このプロジェクトでは既定を 100 ms にしていましたが、その根拠にした測定は
基準クリップを ffplay で再生しながら録画したもので、**録画の負荷で ffplay 自身の映像が
遅れていた**ぶんを「音声が先行している」と読み違えていました。撤回してあります。

測定は「再生アプリの出力遅延 + キャプチャ経路の遅延」を合わせて見ているので、
**普段使う再生アプリで測る**のが正解です。
...
測定を誤りました）。不等間隔なら ±1 秒の範囲で一意に決まります。
